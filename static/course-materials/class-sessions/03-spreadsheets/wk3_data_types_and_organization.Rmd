...
## **Take-home message from the breakout:** Make your data `tidy`.

   1) They should be a rectangle, with only rows and columns.
   1) Each column is a different variable (a thing you’re measuring, like ‘weight’ or ‘temperature’).
   1) One row per observation. Each cell has only one value.
   1) Use short meaningful column names with no spaces.
   1) Use consistent names, abbreviations/codes, and capitalization.
   1) Use good null values (not -999, blanks ok, some prefer `NA` or similar but this can be language specific).
   1) Write dates as YYYY-MM-DD. Better still have separate columns for Year, Month, and Day.
   1) Use one table for each category of data to avoid duplicated chunks of data and to simplify corrections (e.g., taxonomy).
   1) Including all data can lead to errors and takes up more space.
   1) It is also faster to enter (no repretitive typing or cut-paste).
   1) Once you are done with data entry, leave the raw data raw - don’t change it! Save it as 'read only' and make corrections using scripting!
   1) **Tidy data are ideally suited to working with computers.**  
   1) **Entrering data in tidy format will make it much easier to analyze.**  
   1) **Collecting data in tidy format makes it easier to enter data in tidy format.**   
